...
export const handler = async (event: SQSEvent | object, context: object) => {
  ...
  const [postgreHost, postgrePort, postgreUser, postgrePassword, postgreDatabase, riotApiKey, matchServiceRegion, summonerServiceRegion, tftSetVersion, username] = await Promise.all([EnvironmentProvider.get(Parameters.POSTGRES_HOST), EnvironmentProvider.get(Parameters.POSTGRES_PORT), EnvironmentProvider.get(Parameters.POSTGRES_USER), EnvironmentProvider.get(Parameters.POSTGRES_PASSWORD), EnvironmentProvider.get(Parameters.POSTGRES_DB), EnvironmentProvider.get(Parameters.RIOT_API_KEY), EnvironmentProvider.get(Parameters.MATCH_SERVICE_REGION), EnvironmentProvider.get(Parameters.SUMMONER_SERVICE_REGION), EnvironmentProvider.get(Parameters.TFT_SET_VERSION), EnvironmentProvider.get(Parameters.TFT_USERNAME),]);
  ...
  VectorDBPool.getInstance({
    ...
    database: postgreDatabase! as string, // Prevents errors causing from RDS connection. RDS has obligation of using SSL.
    ssl: {
  ...
  const compositionVectorCalculator = new CompositionVectorCalculator(traitsMappingManager, championsMappingManager);

  const iteration = {
    start: 0, count: 20, isFinished: false
  }
      ...
      try {
        ...
        for (const participantPuuid of participants) {
          ...
          if (!summoner) {
            ...
            summoner = {
              summonerid: summonerDetails.id, summonerpuuid: summonerDetails.puuid, summonername: summonerDetails.name,
            }
          ...
          const traitsVector = compositionVectorCalculator.calculateTraitsVector(traits);
          const championsVector = compositionVectorCalculator.calculateChampionsVector(units);
